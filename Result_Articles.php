...
// test to make sure that we selected a radio button ... 
	$optArticle=trim($_GET['optArticle']);  
	$SearchText=trim($_GET['SearchArticle']);  
	...
	*/
	//$where="";
	//if (isset($_POST['optArticles']) || isset($_POST['ArticlesText'])){
	...
	if  (isset($_GET['where'])){
		...
		}elseif ($optArticle=="AU") {
			$where=" AND A.Author LIKE '%{$SearchText}%'";
		}else{
			$where=" AND (A.Title LIKE '%{$SearchText}%'  OR A.Author LIKE '%{$SearchText}%'  OR A.Article LIKE '%{$SearchText}%')";
		}
	}
...
/* Setup vars for query. */
	$targetpage = "Result_Articles.php"; 	//your file name  (the name of this file) 
...
//if (isset($_GET['optArticle']) || isset($_GET['SearchText'])){


	//echo nl2br($optArticle);
	...
	*/
    ...
    // here we began a table and each row in the database will be displayed as a raw in the table  
		while($row = mysql_fetch_array($result)){
			echo '<b><a href="Result_Article.php?articleid='.$row[0].'">'.$row[1].'</a></b></br></br>';
